...
def calc_R0(voltage, current, pontos_pulsos):
    ...
    for i, pts in enumerate(pontos_pulsos):
        ...
        # Filtro: só calcula se for um degrau real de corrente (> 0.5 Ampere)
        if delta_i_ab > 0.5:
    ...
    R0_values = (R0_valores_ab + R0_valores_cd) / 2
    R0_mean = np.nanmean(R0_values)
    R0_median = np.nanmedian(R0_values)
    
    return R0_mean, R0_median, R0_valores_ab, R0_valores_cd, R0_values
