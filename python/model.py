...
def simular_bateria_continua(time, voltage, current, params, R0, func_ocv, popt_ocv, Qn, soc_inicial=1.0):
    """
    ...
    plt.title(f"Validação do Modelo 2RC (RMSE: {rmse_mv:.2f} mV)")
    plt.xlabel('Tempo (s)')
    plt.ylabel('Tensão (V)')
    plt.legend()
    plt.grid(True)
